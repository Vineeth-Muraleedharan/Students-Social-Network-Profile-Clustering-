"""
Student Social Network Profile - Clustering App
...
# ── Page Config ────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Students' Social Network Profile Clustering",
    page_icon="🎓",
    layout="wide",
...
<style>
    ...
    .main-header {
        ...
        padding: 1.2rem; border-radius: 10px; margin-bottom: 1rem; text-align: center;
    }
    .main-header h1 { color: white; font-size: clamp(1.2rem, 4vw, 1.8rem); margin: 0; }
    .main-header p  { color: #BDD7EE; font-size: clamp(0.7rem, 2.5vw, 0.85rem); margin: 0.3rem 0 0 0; }
    .sec {
        background: #EBF0FA; border-left: 4px solid #2E5496;
        padding: 0.4rem 0.8rem; border-radius: 4px; font-weight: bold;
        color: #1F3864; margin: 0.8rem 0 0.5rem 0;
        font-size: clamp(0.8rem, 2.5vw, 0.95rem);
    }
    .box-green {
        background: #E8F5E9; border-left: 4px solid #4CAF50;
        padding: 0.7rem 0.9rem; border-radius: 4px;
        color: #1B5E20; margin: 0.5rem 0; font-size: clamp(0.75rem, 2.2vw, 0.88rem);
    }
    .box-blue {
        background: #E3F2FD; border-left: 4px solid #1565C0;
        padding: 0.7rem 0.9rem; border-radius: 4px;
        color: #0D47A1; margin: 0.5rem 0; font-size: clamp(0.75rem, 2.2vw, 0.88rem);
    }
    .stTabs [data-baseweb="tab-list"] { gap: 4px; overflow-x: auto; flex-wrap: nowrap; }
    .stTabs [data-baseweb="tab"] {
        background: #EBF0FA; border-radius: 6px 6px 0 0;
        padding: 6px 12px; font-weight: 600; color: #2E5496;
        white-space: nowrap; font-size: clamp(0.7rem, 2.5vw, 0.85rem);
    }
    .stTabs [aria-selected="true"] { background: #2E5496 !important; color: white !important; }
...
<div class="main-header">
    ...
    <p>Applied Data Science, ML & AI &nbsp;|&nbsp; E&ICT Academy, IIT Guwahati
    &nbsp;|&nbsp; KMeans · Hierarchical · DBSCAN</p>
</div>
""", unsafe_allow_html=True)
...
with st.expander("⚙️ Model Settings", expanded=False):
    ...
    with c1:
        ...
        pca_var   = st.slider("PCA variance threshold (%)", 80,  99,  90)
        samp_size = st.slider("Silhouette sample size",  1000, 5000, 3000, 500)
    with c2:
        st.markdown("**DBSCAN**")
        eps_pct  = st.slider("Eps percentile",  80, 98, 90)
        min_samp = st.slider("Min samples",      3, 20,  5)
...
def load_and_preprocess(file):
    ...
    for col in numeric_cols:
        Q1 = df_t[col].quantile(0.25); Q3 = df_t[col].quantile(0.75)
        IQR = Q3 - Q1
...
with tab2:
    ...
    with st.spinner("Finding optimal K..."):
        K_range, inertias, sil_scores = find_best_k(X_pca, k_max, samp_size)
    ...
    with c2:
        fig, ax = plt.subplots(figsize=(5, 3))
        ax.plot(K_range, sil_scores, marker='o', color='mediumseagreen', linewidth=2)
    ...
    for c in range(best_k):
        ...
        ax.scatter(X_2d[mask,0], X_2d[mask,1],
                   ...
                   color=plt.cm.tab10.colors[c], alpha=0.5, s=10)
    ...
    with c2:
        ...
        dist.plot(kind='bar', x='Cluster', y='Count', ax=ax,
                  color=list(plt.cm.tab10.colors[:best_k]), edgecolor='white', legend=False)
    ...
    theme_profile.T.plot(kind='bar', ax=ax, colormap='tab10', edgecolor='white', width=0.7)
    ...
    with c1:
        ...
        plt.sca(ax); plt.title('Age by Cluster', fontweight='bold')
        ax.set_xlabel('Cluster'); plt.suptitle('')
        plt.tight_layout(); st.pyplot(fig, use_container_width=True); plt.close()
    with c2:
        fig, ax = plt.subplots(figsize=(5, 4))
        df.groupby('Cluster')['gender'].value_counts(normalize=True).unstack().plot(
            kind='bar', ax=ax, colormap='Set2', edgecolor='white', width=0.6)
        ax.set_title('Gender Ratio by Cluster', fontweight='bold')
        ax.tick_params(axis='x', rotation=0); ax.legend(title='Gender', fontsize=8)
        plt.tight_layout(); st.pyplot(fig, use_container_width=True); plt.close()
    ...
    for row_themes in [theme_list[:3], theme_list[3:]]:
        ...
        for col_widget, theme in zip(cols, row_themes):
            ...
            trend.plot(ax=ax, marker='o', linewidth=1.5, colormap='tab10')
...
with tab3:
    ...
    # KMeans scores (already have km_labels)
    t0      = time.time()
    ...
    ax.axhline(y=linked[-(best_k)+1, 2], color='red', linestyle='--',
    ...
    with c1:
        ...
        ax.axhline(y=sug_eps, color='red', linestyle='--', label=f'eps={sug_eps}')
        ax.set_title('K-Distance Plot (k=5)', fontweight='bold', fontsize=9)
        ax.set_xlabel('Points'); ax.set_ylabel('5-NN Distance'); ax.legend(fontsize=8)
        plt.tight_layout(); st.pyplot(fig, use_container_width=True); plt.close()
    with c2:
    ...
    for col_w, (labels, title) in zip(cols, [
        ...
        for i, lbl in enumerate(sorted(set(labels))):
            mask  = labels == lbl
            color = 'black' if lbl == -1 else plt.cm.tab10(i % 10)
    ...
    for vals, title in [
        ...
        bars = ax.bar(methods_lbl, vals, color=colors_m, edgecolor='white', width=0.4)
